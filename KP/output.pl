...
relative(daughter,Daughter,Parent) :- daughter(Daughter,Parent).

relative(sister,Sister,Brother) :- sister(Sister,Brother).

relative(brother,Brother,Sister) :- brother(Brother,Sister).

relative(father,Father,Child) :- father(Child,Father).

relative(mother,Mother,Child) :- mother(Child,Mother).

relative(husband,Husband,Wife) :- parents(_,Husband,Wife), Husband\=father_NAN, Wife\=mother_NAN, !.

relative(wife,Wife,Husband) :- parents(_,Husband,Wife), Husband\=father_NAN, Wife\=mother_NAN, !.

relative(X, First, Second) :- 
	relative(Y,First,Second), relative(A,Second,B), A\=Y,
	relative(C,First,B), relative(D,B,First), my_append(_,[C-D],X), !.
